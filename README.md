...
### SRT bonding relay: SRS idle-publisher timeout

SRS has a hardcoded **5-second idle publisher timeout** (`srs_protocol_srt.cpp`, `recv_timeout_ = 5 * SRS_UTIME_SECONDS`) that is not configurable via `srs.conf`. When `srt-live-transmit` is used as a bonding relay, it connects to SRS immediately at startup — before the encoder has connected to the relay's listener port. SRS sees a publisher connection with no data arriving and kicks it after 5 seconds. The relay crashes, restarts after 5 seconds, and the cycle repeats indefinitely. The encoder can never connect because the relay's listener port is down during each restart window.

This does **not** affect direct encoders (vMix, OBS, ffmpeg) publishing SRT straight to SRS port 10080 — they send video frames the moment the connection is established, so the 5-second timer never fires. It only affects `srt-live-transmit` acting as an intermediary, because `srt-live-transmit` connects to SRS before it has received anything from the upstream encoder.

**Workaround:** Start the encoder first so it is already retrying connections to the relay port. When `srt-live-transmit` then starts and opens its listener, the encoder connects within ~1–2 seconds and data reaches SRS well inside the 5-second window. This is fragile — it only works for the initial connection and breaks again after any relay crash/restart.

**Proper fix:** Patch `build/srs-src/trunk/src/protocol/srs_protocol_srt.cpp` line ~708 and change `5` to `30` (or higher), then rebuild SRS. This gives the relay enough time to accept the encoder's bonded connection and start forwarding data before SRS gives up. Any direct encoder is unaffected because it sends data immediately regardless of the timeout value.

### SRT bonding relay: ffmpeg cannot accept bonded SRT group connections

ffmpeg's SRT protocol handler does **not** set `SRTO_GROUPCONNECT=1` on the listener socket even when `groupconnect=1` is passed as a URL parameter — the option is not in ffmpeg's recognised SRT option table and is silently ignored. Without `SRTO_GROUPCONNECT`, the SRT listener rejects bonded (group) connection attempts at the handshake level. An AJA Bridge Live or similar encoder configured to send two redundant paths (e.g. via `eno1` and `eno2`) using SRT broadcast bonding will never be able to connect to an ffmpeg listener.

`srt-live-transmit` does support `groupconnect=1` correctly and is the right tool for the encoder-facing side of the bonding relay. The SRS-idle-timeout issue above is the reason the relay was temporarily switched to ffmpeg — ffmpeg's format probe delays its output (SRS) connection until data is already flowing, which avoids the idle-publisher timeout. The two-process design that fixes both problems cleanly is: `srt-live-transmit` accepts the bonded encoder connection and forwards a plain SRT stream to an internal port; ffmpeg reads from that internal port and pushes to SRS only once data arrives.
