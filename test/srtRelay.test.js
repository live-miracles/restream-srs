...
describe('SRT relay service', () => {
                    ...
                    streamStates: [
                        {
                            ...
                            inputRttMs: 4.25,
                            outputRttMs: 5.1,
                            outputSentPacketsTotal: 40,
                            outputSendLossTotal: 1,
                            outputSendDropTotal: 0,
                            outputRetransTotal: 2,
                            legs: [
                                {
                                    ip: '203.0.113.10',
                                    port: 10081,
    ...
    test('clears stale per-stream errors after the relay restarts', async () => {
        const erroredState = {
            ...
            inputRttMs: 12.5,
            outputRttMs: 13.1,
            outputSentPacketsTotal: 880,
            outputSendLossTotal: 3,
            outputSendDropTotal: 1,
            outputRetransTotal: 5,
            legs: [],
    ...
    test('keeps per-stream status lookup stable with 20 concurrent streams', async () => {
        const streamStates = Array.from({ length: 20 }, (_, idx) => {
            ...
            return {
                ...
                recvDropTotal: idx % 2,
                retransTotal: idx + 2,
                inputRttMs: 2.5 + idx / 10,
                outputRttMs: 2.5 + idx / 10,
                outputSentPacketsTotal: (idx + 1) * 90,
                outputSendLossTotal: idx % 2,
                outputSendDropTotal: idx % 4 === 0 ? 1 : 0,
                outputRetransTotal: idx + 1,
                legs: [],
        ...
        assert.deepEqual(service.getStreamStatus('#!::r=live/key20,m=publish'), {
            ...
            inputRttMs: 4.4,
            outputRttMs: 4.4,
            outputSentPacketsTotal: 1800,
            outputSendLossTotal: 1,
            outputSendDropTotal: 0,
            outputRetransTotal: 20,
            legs: [],
            lastErrorAt: null,
            lastError: null,
        });
